...
function normalizeGuild(raw: unknown): Guild | null {
  ...
  return {
    ...
    emblemIcon:
      typeof data.emblemIcon === 'string' && data.emblemIcon
        ? data.emblemIcon
        : '🚩',
    emblemBg:
...
export const guildStore = {
  ...
  createGuild(params: { name: string; tag: string; emblemIcon?: string; emblemBg?: string }): boolean {
    const state = store.getSnapshot();
    if (state.guildId) {
  ...
  updateEmblem(icon: string, bg: string): boolean {
    const state = store.getSnapshot();
    const role = this.getMyRole();
    if (!state.guildId || !role || !isLeadershipRole(role)) return false;
    updateGuild(state.guildId, (g) => ({
