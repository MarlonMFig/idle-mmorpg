...
export async function GET(req: Request): Promise<Response> {
  return withSocialApi(
    req,
    { auth: true, rateKey: 'guild-get', rateLimit: 120 },
    async ({ db, playerId }) => {
      const url = new URL(req.url);
      const guildId = url.searchParams.get('guildId');
      const q = url.searchParams.get('q');
      const mine = url.searchParams.get('mine') === '1';
      const visibleGuild = (guild: Awaited<ReturnType<typeof guilds.getGuild>>) => {
        if (!guild || !playerId) return guild;
        const member = guild.members.find((row) => row.playerId === playerId);
      ...
      if (mine && playerId) {
        ...
        return jsonOk({ guildId: id, guild: visibleGuild(guild) });
      }

      if (guildId) {
        const guild = await guilds.getGuild(db, guildId);
        return jsonOk({ guild: visibleGuild(guild) });
      }
...
export async function POST(req: Request): Promise<Response> {
  return withSocialApi(
    req,
    { auth: true, rateKey: 'guild-mutate', rateLimit: 40 },
    async ({ db, playerId, nickname }) => {
      ...
      switch (action) {
        ...
        case 'applications': {
          const apps = await guilds.getApplications(db, String(body.guildId ?? ''), playerId);
