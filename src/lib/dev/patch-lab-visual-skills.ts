...
export function insertLabVisualSkill(def: SkillDefinition): { relativePath: string } {
...
export function patchLabVisualSkillElement(skillId: string, element: string): { relativePath: string } {
  const absPath = path.join(process.cwd(), FILE_REL);
  assertWritableSourcePath(absPath);
  const source = fs.readFileSync(absPath, 'utf8');
  ...
  if (!idMatch || idMatch.index == null) return { relativePath: FILE_REL };
  const open = source.lastIndexOf('{', idMatch.index);
  const end = matchingBracket(source, open);
  if (open < 0 || end < 0) return { relativePath: FILE_REL };
  ...
  fs.writeFileSync(absPath, `${source.slice(0, open)}${block}${source.slice(end + 1)}`, 'utf8');
  return { relativePath: FILE_REL };
}
