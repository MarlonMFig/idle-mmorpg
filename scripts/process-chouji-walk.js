 ...
 *
 * npm run chouji:walk
 * Fonte: assets/naruto-source/nu/chouji-walk-sheet.png
 */
const fs = require('fs');
...
function cellsFromContent(data, w, band) {
  ...
  // merge nearly-touching (limb contact)
  const merged = [];
  for (const c of runs) {
...
async function main() {
  ...
  console.log(
    `chouji walk ${info.width}x${info.height} band ${band.t}-${band.b} cells=${cells.length} (hint ${HINT_EXPECTED})`,
  ...
  const entry = {
    image: '/sprites/player/chouji/walk.png',
