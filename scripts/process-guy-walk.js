/**
 * Might Guy walk — 6-frame side walk RIGHT, green chroma exterior only.
 * Olive gi preserved (not pure screen green). No black flood/peel.
 *
 * npm run guy:walk
 * Fonte: assets/naruto-source/nu/guy/walk-sheet.png
 * Output: public/sprites/player/guy/walk.png
 */
const fs = require('fs');
...
const INPUT = path.join(ROOT, 'assets', 'naruto-source', 'nu', 'guy', 'walk-sheet.png');
...
function cellsFromContent(data, w, band) {
  ...
  // slightly lower thr: narrow profile walk poses
...
function extractCell(data, w, cell) {
  ...
  for (let y = 0; y < fh; y += 1) {
    for (let x = 0; x < fw; x += 1) {
      ...
      const g = data[si + 1];
      const b = data[si + 2];
      if (isGreenBg(r, g, b) && greenness(r, g, b) >= 40 && r <= 40 && b <= 40) {
        frame[di + 3] = 0;
        continue;
      }
      frame[di] = r;
...
async function scaleFrames(frames, cellW, cellH, contentHeight, scaleOpts = { mode: 'match', metaPath: META_JSON, idleKey: 'guy-idle' }) {
  ...
  for (const frame of frames) {
    ...
    for (let y = 0; y < outH; y += 1) {
      for (let x = 0; x < outW; x += 1) {
        ...
        if (edge && isGreenBg(r, g, b) && gn >= 40 && r <= 50 && b <= 50) {
          d[i] = 0;
          d[i + 1] = 0;
          d[i + 2] = 0;
          d[i + 3] = 0;
...
function qaMetrics(sheetData, sheetW, fh, fw, n) {
  ...
  for (let f = 0; f < n; f += 1) {
    ...
    footY.push(maxFoot);
    if (pix < 70) throw new Error(`Frame ${f} too empty (${pix}px)`);
...
async function writeQaCrops(sheet, scaled, qaDir) {
  ...
  for (let fi = 0; fi < scaled.frames.length; fi += 1) {
    ...
    await sharp(magF, {
      ...
      .png()
      .toFile(path.join(qaDir, `walk-frame-${fi}-mag-x6.png`));
  }
}
...
async function main() {
  ...
  console.log(
    `guy walk ${info.width}x${info.height} band ${band.t}-${band.b} cells=${cells.length} (hint ${HINT_EXPECTED})`,
    cells.map((c) => c.r - c.l).join(','),
  );
  if (cells.length !== HINT_EXPECTED) {
    throw new Error(`Expected ${HINT_EXPECTED} walk frames, got ${cells.length}`);
  }
  ...
  for (let i = 0; i < cut.length; i += 1) {
    ...
    if (n < 70) throw new Error(`Frame ${i} empty after chroma (${n}px)`);
  }
  ...
  const entry = {
    image: '/sprites/player/guy/walk.png',
    ...
    scale: scaled.scale,
    source: 'assets/naruto-source/nu/guy/walk-sheet.png',
    frameRate: FRAME_RATE,
    residualGreen: qa.residualGreen,
    pureBlack: qa.pureBlack,
    oliveGi: qa.oliveGi,
    note: '6-frame side walk RIGHT; exterior green only; olive gi preserved',
  };
