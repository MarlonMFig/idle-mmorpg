/**
 * Folha horizontal do idle Sasuke (fundo verde #008000) →
 * public/sprites/player/sasuke/idle.png
 *
 * Uso: node scripts/process-sasuke-idle-sheet.js
 * Fonte: assets/naruto-source/nu/sasuke-idle-sheet.png
 */
const fs = require('fs');
...
const INPUT = path.join(ROOT, 'assets', 'naruto-source', 'nu', 'sasuke-idle-sheet.png');
const OUT_DIR = path.join(ROOT, 'public', 'sprites', 'player', 'sasuke');
const TARGET_BODY_H = 48; // CHARACTER_DISPLAY_HEIGHT → scale 1x no jogo
...
async function main() {
  ...
  meta['sasuke-idle'] = {
    ...
    contentHeight,
    source: 'sasuke-idle-sheet.png',
    frameRate: 7,
  };
  fs.writeFileSync(metaPath, `${JSON.stringify(meta, null, 2)}\n`);

  console.log(
    `-> idle.png ${sheetW}x${outH} fw=${outW} fh=${outH} count=${frames.length} contentH=${contentHeight}`,
  );
}
