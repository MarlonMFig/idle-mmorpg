/**
 * Sequência PNG do Chidori (fundo preto + contorno verde) →
 * public/sprites/player/sasuke/sasuke-chidori.png
 *
 * Uso: node scripts/process-sasuke-chidori.js
 *      npm run sasuke:chidori
 *
 * Mesma lógica de flood/peel do body (não apaga stroke preto do corpo).
 * Preserva ciano/branco do relâmpago (só chroma verde “puro”).
 */
const fs = require('fs');
...
const INPUT_DIR = path.join(ROOT, 'assets-src', 'sasuke-chidori');
const OUT_DIR = path.join(ROOT, 'public', 'sprites', 'player', 'sasuke');
const OUT_NAME = 'sasuke-chidori';
/** Altura alvo do corpo (px), alinhada ao idle lateral (~48). */
const TARGET_BODY_H = 48;
const EDGE_PEEL_PASSES = 2;
/** Largura máxima da strip (WebGL / memória). */
const MAX_SHEET_W = 4096;

function isGreen(r, g, b) {
  // Contorno green-screen: R e B baixos, G dominante (não ciano elétrico).
  if (g >= 55 && r <= 70 && b <= 70 && g >= r + 18 && g >= b + 18) return true;
  if (g >= 40 && r <= 50 && b <= 50 && g > r + 10 && g > b + 10) return true;
  // Franja verde-limão do outline (#0080xx) sem azul de chidori.
...
function keyFrame(data, w, h) {
  ...
  // Threshold baixo: faíscas/traços do Chidori soltos não somem.
  out = keepSignificantComponents(out, w, h, 0.006, 18);
...
/**
 * Altura do corpo (tons médios), para escalar o personagem sem o VFX
 * enorme do Chidori encolher o Sasuke na tela.
 */
function bodyContentHeight(frame, w, h, box) {
  ...
  for (let y = box.minY; y <= box.maxY; y += 1) {
    for (let x = box.minX; x <= box.maxX; x += 1) {
      ...
      const g = frame[i + 1];
      const b = frame[i + 2];
      // Ciano/branco elétrico: ignora na medição do corpo.
...
function normalize(frames, widths, heights) {
  ...
  // Corpo nos primeiros frames de carga (sem lunge largo).
...
async function writePreview(frames, fw, fh, outPath) {
  const cols = Math.min(11, frames.length);
  const sheet = stitchGrid(frames, fw, fh, cols);
  await sharp(sheet.data, {
    raw: { width: sheet.width, height: sheet.height, channels: 4 },
  })
    .png()
    .toFile(outPath);
  console.log(`  preview: ${path.relative(ROOT, outPath)} ${sheet.width}x${sheet.height}`);
}
...
async function main() {
  ...
  // Impacto no início do lunge (~60% da sequência).
  const hitDelayMs = Math.round(durationMs * 0.55);

  const sheet = stitchAuto(frames, scaled.frameWidth, scaled.frameHeight);
  ...
  const previewPath = path.join(ROOT, 'assets-src', 'sasuke-chidori-preview.png');
  await writePreview(frames, scaled.frameWidth, scaled.frameHeight, previewPath);
  ...
  meta[OUT_NAME] = {
    ...
    source: 'assets-src/sasuke-chidori (frame_001…033)',
  };
  meta['skill-chidori'] = {
    ...meta[OUT_NAME],
    image: file,
    note: 'alias → sasuke-chidori',
  };
  ...
  console.log(
    ...
    `count=${frameCount} contentH=${scaled.contentHeight}`,
