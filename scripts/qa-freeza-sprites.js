/**
 * Strict sprite QA for Freeza packed sheets.
...
const OUT = path.join(ROOT, 'public', 'sprites', 'player', 'freeza');
...
const SHEETS = [
  { file: 'walk.png', metaKey: 'freeza-walk', minOpaque: 80, checkFeet: true },
  { file: 'idle.png', metaKey: 'freeza-idle', minOpaque: 100, checkFeet: true },
  { file: 'combo1.png', metaKey: 'freeza-combo1', minOpaque: 80, checkFeet: false },
  { file: 'combo2.png', metaKey: 'freeza-combo2', minOpaque: 80, checkFeet: false },
  { file: 'combo3.png', metaKey: 'freeza-combo3', minOpaque: 80, checkFeet: false },
  { file: 'hurt.png', metaKey: 'freeza-hurt', minOpaque: 60, checkFeet: false },
  { file: 'death.png', metaKey: 'freeza-death', minOpaque: 40, checkFeet: false },
  { file: 'death-ball.png', metaKey: 'freeza-death-ball', minOpaque: 40, checkFeet: true },
  { file: 'death-ball-fx.png', metaKey: 'freeza-death-ball-fx', minOpaque: 8, checkFeet: false },
];
...
async function analyzeStrip(file, fw, fh, n, minOpaque, checkFeet) {
  ...
  for (let f = 0; f < n; f += 1) {
    ...
    // Edge contact = likely crop cut-off
    if (minX <= 0 || maxX >= fw - 1) {
  ...
  if (contentH.length > 1) {
    ...
    // Death last frame / beam sheets can flatten — skip harsh check on death/fx
    if (!/death\.png|death-ball|hurt\.png|combo\d+\.png/.test(file) && spread > MAX_CONTENT_H_SPREAD) {
  ...
  for (const r of report) {
    ...
    const h =
      r.contentH && r.contentH.length
        ...
        : '';
    console.log(
      `${r.errors.length ? 'FAIL' : 'OK  '} ${r.file}${h}${feet} green=${r.residualGreen ?? 0}`,
    );
