/**
 * Rock Lee — 2ª skill Omote Renge (frame_001…052, fundo verde + preto).
 * assets-src/rock-lee-skill2 → public/sprites/player/rock-lee/omote-renge.png
 *
 * npm run rock-lee:skill2
 *
...
const INPUT_DIR = path.join(ROOT, 'assets-src', 'rock-lee-skill2');
const OUT_DIR = path.join(ROOT, 'public', 'sprites', 'player', 'rock-lee');
const OUT_NAME = 'omote-renge';
...
async function main() {
  ...
  console.log(`rock-lee skill2 (Omote Renge): ${files.length} frames`);
  const cut = [];
  ...
  const entry = {
    ...
    durationMs,
    hitDelayMs,
    source: 'assets-src/rock-lee-skill2 (frame_001…052)',
  };
  ...
  meta['skill-omote-renge'] = { ...entry, note: '2ª skill Rock Lee' };
