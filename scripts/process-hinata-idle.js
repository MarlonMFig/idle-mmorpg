/**
 * Hinata idle — 5-frame Gentle Fist stance, green chroma exterior only.
 * Preserve lavender jacket / pure-black hair (no stripLabels, no black key).
 *
 * npm run hinata:idle
 * Fonte: assets/naruto-source/nu/hinata-idle-sheet.png
 */
const fs = require('fs');
...
const INPUT = path.join(ROOT, 'assets', 'naruto-source', 'nu', 'hinata-idle-sheet.png');
const OUT_DIR = path.join(ROOT, 'public', 'sprites', 'player', 'hinata');
const PREVIEW = path.join(ROOT, 'public', 'sprites', 'player', 'previews', 'hinata.png');
const META_JSON = path.join(OUT_DIR, 'meta.json');
const QA_DIR = path.join(ROOT, 'assets-src', '_qa', 'hinata');
const TARGET_BODY_H = 48;
const FRAME_RATE = 8;
const HINT_EXPECTED = 5;
...
function qaMetrics(sheetData, sheetW, fh, fw, n) {
  ...
  for (let f = 0; f < n; f += 1) {
    ...
    if (frameBlack < 20) {
      throw new Error(`Frame ${f} pure black hair too low (${frameBlack}px)`);
    }
    if (frameHeadHoles > 6) {
...
async function main() {
  ...
  console.log(
    `hinata idle ${info.width}x${info.height} band ${band.t}-${band.b} cells=${cells.length} (hint ${HINT_EXPECTED})`,
    cells.map((c) => c.r - c.l).join(','),
  );
  if (cells.length < 3) throw new Error(`Poucos frames: ${cells.length}`);
  ...
  console.log(
    `QA residualGreen=${qa.residualGreen} opaque=${qa.opaque} pureBlackHair=${qa.pureBlackHair} headInteriorHoles=${qa.headInteriorHoles} footSpread=${qa.footSpread}`,
  );
  if (qa.residualGreen > 0) {
    throw new Error(`QA fail: residual green pixels = ${qa.residualGreen}`);
  }
  if (qa.pureBlackHair < 150) {
    throw new Error(`QA fail: pure black hair nearly gone (${qa.pureBlackHair}px)`);
  }
  if (qa.headInteriorHoles > 12) {
  ...
  const entry = {
    image: '/sprites/player/hinata/idle.png',
    ...
    scale: scaled.scale,
    source: 'hinata-idle-sheet.png',
    frameRate: FRAME_RATE,
    residualGreen: qa.residualGreen,
    pureBlackHair: qa.pureBlackHair,
    headInteriorHoles: qa.headInteriorHoles,
    note: `${scaled.frames.length}-frame Gentle Fist idle stance; green exterior key; no black key`,
  };
