...
async function processCuratedAlphaPack(cfg) {
  ...
  if (nativeOpts) {
    ...
    const walkScale = preferNativeScale(
      walkMaxH > 0 ? idleBodyH / walkMaxH : 1,
    );
    console.log(
  ...
  if (nativeOpts) {
    ...
    for (const k of comboKeyed) {
      const b = bbox(scrub(k.frame), k.width, k.height);
      comboMaxH = Math.max(comboMaxH, b.height);
    }
    const comboScale = preferNativeScale(
      comboMaxH > 0 ? contentHeight / comboMaxH : 1,
    );
    console.log(
      `${id}: combo scale=${comboScale.toFixed(4)} (comboMaxH=${comboMaxH} → contentHeight=${contentHeight})`,
    );
    comboOpts = {
  ...
  if (nativeOpts) {
    ...
    const damageScale = preferNativeScale(
      standingBodyH > 0 ? contentHeight / standingBodyH : 1,
    );
    console.log(
