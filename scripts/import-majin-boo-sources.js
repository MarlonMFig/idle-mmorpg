/**
 * Import Majin Boo sprites from Downloads into
 * assets/dragon-ball-source/nu/majin-boo/{idle,walk,combo,damage,jutsu,vfx}/
 *
 * FLY.zip → walk. ESPECIAL.zip (prefer) / ESPECIAL.png → jutsu.
 * vfx.zip → vfx (projectile + impact).
 *
 *   node scripts/import-majin-boo-sources.js
 *   MAJIN_BOO_SRC=... node scripts/import-majin-boo-sources.js
 */
const fs = require('fs');
...
const DOWNLOAD =
  process.env.MAJIN_BOO_SRC ||
  path.normalize('c:\\Users\\marlo\\Downloads\\SPRITES JOGO\\MAJIN BOO');
const DEST = path.join(ROOT, 'assets', 'dragon-ball-source', 'nu', 'majin-boo');
...
/** ESPECIAL.png fallback — 3 rows de cast (6f cada); bandas extras = debris (ignorar). */
async function sliceEspecialSheet(sheetPath, outDir) {
  ...
  console.log(`ESPECIAL.png sliced: ${frames.length} frames (3 body rows)`);
...
async function main() {
  ...
  const especialZip = path.join(DOWNLOAD, 'ESPECIAL.zip');
  const especialPng = path.join(DOWNLOAD, 'ESPECIAL.png');
  let jutsuN = 0;
  if (fs.existsSync(especialZip)) {
    ...
    jutsuN = await normalizeFolder(especialRaw, path.join(DEST, 'jutsu'), 'jutsu (ESPECIAL.zip)');
  } else if (fs.existsSync(especialPng)) {
    jutsuN = await sliceEspecialSheet(especialPng, path.join(DEST, 'jutsu'));
  } else {
    throw new Error('Missing ESPECIAL: need ESPECIAL.zip or ESPECIAL.png under ' + DOWNLOAD);
  }

  const vfxZip = path.join(DOWNLOAD, 'vfx.zip');
  let vfxN = 0;
  if (fs.existsSync(vfxZip)) {
    const vfxRaw = path.join(rawRoot, 'vfx-zip');
    fs.mkdirSync(vfxRaw, { recursive: true });
    extractZip(vfxZip, vfxRaw);
    vfxN = await normalizeFolder(vfxRaw, path.join(DEST, 'vfx'), 'vfx (vfx.zip)');
  } else {
    console.warn('skip missing vfx.zip — especial-fx will fail until present');
  }
