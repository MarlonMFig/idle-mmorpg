/**
 * Jiraiya combo attack — alpha-only frames (18f → 5+5+8).
 ...
 * Output: public/sprites/player/jiraiya/{combo1,combo2,combo3,attack}.png
 */
const fs = require('fs');
...
/** Three multi-hit beats: hit1 | hit2 | finisher (5+5+8). */
const SLICES = [
  { name: 'combo1', from: 0, to: 5, note: 'prep + first strike' },
  { name: 'combo2', from: 5, to: 10, note: 'mid combo thrusts' },
  { name: 'combo3', from: 10, to: 18, note: 'overhead finisher + recovery' },
];
...
function isBodyPixel(r, g, b) {
  if (isWhiteHair(r, g, b) || isTanEnergy(r, g, b)) return false;
...
function bodyAnchor(frame, w, h) {
  ...
  for (let y = 0; y < h; y += 1) {
    for (let x = 0; x < w; x += 1) {
      ...
      const g = frame[i + 1];
      const b = frame[i + 2];
      if (!isTanEnergy(r, g, b) && y > feetBodyY) feetBodyY = y;
    }
  }
  if (maxY < 0) {
  ...
  if (m.n < 30) {
    mode = 'fullNoHairEnergy';
    m = massIn((r, g, b) => !isWhiteHair(r, g, b) && !isTanEnergy(r, g, b), minY, feetBodyY);
  }
  if (m.n < 15) {
    mode = 'full';
  ...
  return frames.map((frame) => {
    ...
    for (let y = 0; y < fh; y += 1) {
      for (let x = 0; x < fw; x += 1) {
        const i = (y * fw + x) * 4;
        if (frame[i + 3] < ALPHA_KEEP) continue;
        if (isTanEnergy(frame[i], frame[i + 1], frame[i + 2])) continue;
        if (y < minY) minY = y;
        if (y > maxY) maxY = y;
      }
    }
...
function countTanEnergy(sheetData) {
  let n = 0;
  for (let i = 0; i < sheetData.length; i += 4) {
    if (sheetData[i + 3] < ALPHA_KEEP) continue;
    if (isTanEnergy(sheetData[i], sheetData[i + 1], sheetData[i + 2])) n += 1;
  }
  return n;
}
  ...
  if (walkScaleInfo) {
    ...
    console.log(
      `NOTE: walk scale ${walkScaleInfo.scale.toFixed(6)} → body≈${projected.toFixed(1)}px ` +
        `(need ~${TARGET_BODY_H}); using body-match nearest ${bodyMatch.toFixed(6)}`,
    );
  }
  return {
    scale: bodyMatch,
    source: `body-match-nearest→${TARGET_BODY_H}`,
...
async function main() {
  ...
  const tanEnergy = countTanEnergy(fullSheet.data);
  ...
  console.log(
    `QA residualGreen=${qa.residualGreen} opaque=${qa.opaque} pureBlack=${qa.pureBlack} whiteHair=${whiteHair} tanEnergy=${tanEnergy} footSpread=${qa.footSpread}`,
  );
  console.log(
  ...
  for (const slice of SLICES) {
    ...
    const entry = {
      image: `/sprites/player/jiraiya/${slice.name}.png`,
      ...
      source: 'assets/naruto-source/nu/jiraiya/combo/frame_001..018.png',
      ...
      contentHeights,
      note: `${slice.note} (${frames.length}f); alpha-only; body-lock; nearest→${TARGET_BODY_H} contentH=${scaled.contentHeight}`,
    };
    updateMeta(META_JSON, `jiraiya-${slice.name}`, entry);
    wire[slice.name] = {
  ...
  updateMeta(META_JSON, 'jiraiya-attack', {
    image: '/sprites/player/jiraiya/attack.png',
    frameWidth: scaled.frameWidth,
    frameHeight: scaled.frameHeight,
    ...
    note:
      ...
      `alpha-only; body-lock; nearest scale=${scaled.scale.toFixed(4)} contentH=${scaled.contentHeight}`,
  });

  console.log(
    `-> attack.png ${fullSheet.width}x${fullSheet.height} fw=${scaled.frameWidth} fh=${scaled.frameHeight} ` +
      `n=${scaled.frames.length} contentH=${scaled.contentHeight} scale=${scaled.scale.toFixed(6)} kernel=nearest`,
  );
  ...
  console.log(
    ...
    JSON.stringify({
      ...
      contentHeight: scaled.contentHeight,
      scale: scaled.scale,
      scaleMethod: Math.abs(scaled.scale - 1) < 1e-6 ? '1:1-noscale' : 'nearest-body-match',
      residualGreen: qa.residualGreen,
      whiteHair,
      tanEnergy,
