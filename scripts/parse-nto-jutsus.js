/**
 * Minera os jutsus do servidor "NTO War Legends" (spells.xml) e gera um
 * catálogo de REFERÊNCIA em src/data/nto-jutsu-reference.ts.
 *
 * Não é o catálogo final do jogo: dano é estimado (os .lua do TFS usam
 * fórmulas/storages que não portam direto). Serve para curar manualmente
 * para dentro de src/data/skills.ts.
 */
const fs = require('fs');
const path = require('path');

const SPELLS_XML =
  'C:/Users/marlo/Downloads/NTO WAR LEGENDS/NTO WAR LEGENDS/Servidor/data/spells/spells.xml';
const OUT_FILE = path.join(__dirname, '..', 'src', 'data', 'nto-jutsu-reference.ts');

/** name → element por palavra-chave (natureza do chakra). */
const ELEMENT_RULES = [
  ['fire', /katon|fire|fireball|flame|amateras|goukakyu|housenka|karyu|bijuu\s*dama|bijudama/i],
  ['water', /suiton|water|mizu|kirigakure|suirou|daibakufu/i],
  ['wind', /futon|fuuton|wind|rasen\s*shuriken|rasenshuriken|kamaitachi|renkudan/i],
  ['earth', /doton|earth|doryuheki|iwa|golem|shinju|mokuton|wood/i],
  ['lightning', /raiton|lightning|chidori|raikiri|kirin|thunder|raigeki/i],
  ['yin', /genjutsu|tsukuyomi|kotoamatsukami|shadow|kagemane|nara/i],
  ['yang', /rasengan|taijutsu|hachimon|gate|lariat|punch|puch|kick|chakra\s*puch/i],
];
...
for (const tag of tags) {
  ...
  // ou spells de script agressivos com alvo/alcance.
  const hasCharPath = event === 'script' && value.includes('/');
  ...
  const character = hasCharPath ? value.split('/')[0] : 'todos';
...
const header = `// GERADO por scripts/parse-nto-jutsus.js — catálogo de REFERÊNCIA.
...
export const NTO_JUTSU_REFERENCE: readonly NtoJutsuReference[] = ${JSON.stringify(
  entries,
  null,
  2,
)};
